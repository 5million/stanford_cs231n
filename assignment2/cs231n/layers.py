...
def softmax_loss(x, y):
    ...
    exp_res = np.exp(scores)
    prob = exp_res / np.sum(exp_res, axis=1).reshape(-1, 1)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - sample_mean), axis=0) * (-0.5) * np.power((sample_var + eps), -1.5)
    dmean = dvar * (-2) * np.sum(x - sample_mean, axis=0) / N - np.sum(dx_norm, axis=0) / np.sqrt(sample_var + eps)
    dx = dx_norm / np.sqrt(sample_var + eps) + dmean / N + dvar * 2 * (x - sample_mean) / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range (N):
      for j in range(F):
        for k in range(Ho):
          for p in range(Wo):
            out[i, j, k, p] = np.sum(x_pad[i, :, k*stride:k*stride+HH, p*stride:p*stride+WW] * \
              w[j, :, :, :]) + b[j]
      ...
      for j in range(C):
        for k in range(HH):
          for l in range(WW):
            for ho in range(Ho):
              for wo in range(Wo):
                dw[i, j, k, l] += np.sum(dout[:, i, ho, wo] * x_pad[:, j, ho*stride+k, wo*stride+l], axis=0)
