...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        cur_h, _ = rnn_step_forward(x[:, i, :], cur_h, Wx, Wh, b)
        h[:, i, :] = cur_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        step_cache = (x[:, i, :], prev_h, Wx, Wh, b, next_h)
        next_h = prev_h
        dnext_h = dh[:, i, :] + dprev_h
        dx[:, i, :], dprev_h, cur_dWx, cur_dWh, cur_db = rnn_step_backward(dnext_h, step_cache)
        dWx += cur_dWx
        dWh += cur_dWh
        db += cur_db
    dh0 = dprev_h
...
def word_embedding_forward(x, W):
    ...
    V, _ = W.shape
    x_one_hot = np.eye(V)[x]
    out = x_one_hot.dot(W)
    cache = (x, x_one_hot, W, out)
...
def word_embedding_backward(dout, cache):
    ...
    x, x_one_hot, W, out = cache
    N = x.shape[0]
    dW = np.zeros_like(W)
    for i in range(N):
        dW += x_one_hot[i].transpose(1, 0).dot(dout[i])
